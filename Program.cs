...
namespace ConsoleApp1
{
    class Program
    {
        ...
        static async Task ViewAllAppointmentsAdmin()
        {
            ...
            {
                Console.WriteLine("Khong co lich hen nao.");
                Console.WriteLine("Nhan phim bat ky de quay lai...");
                Console.ReadKey();
                return;
            }
            Console.WriteLine("Danh sach tat ca lich hen:");
            Console.WriteLine("--------------------------------------------------------------------------------");
            Console.WriteLine("STT | AppointmentID                      | CustomerID                        | BarberID                         | ServiceID                        | DateTime            | Status");
            Console.WriteLine("--------------------------------------------------------------------------------");
            int index = 1;
            foreach (var appt in appointments)
            {
                Console.WriteLine($"{index,-4}| {appt.AppointmentID,-32} | {appt.CustomerID,-30} | {appt.BarberID,-30} | {appt.ServiceID,-30} | {appt.AppointmentDateTime,-19} | {appt.Status}");
                index++;
            }
            Console.WriteLine("--------------------------------------------------------------------------------");
            Console.WriteLine("Nhan phim bat ky de quay lai...");
        ...
        // The rest of the code from line 108 onwards goes here inside the Program class and namespace
        ...
        static async Task UpdateAppointmentStatusAdmin()
        {
            var appointments = new List<Entities.Appointment>();
            using (var connection = new MySql.Data.MySqlClient.MySqlConnection(DataAccess.DatabaseConfig.ConnectionString))
            {
            ...
            Console.WriteLine("Danh sach lich hen:");
            Console.WriteLine("--------------------------------------------------------------------------------");
            Console.WriteLine("STT | AppointmentID                      | CustomerID                        | BarberID                         | ServiceID                        | DateTime            | Status");
            Console.WriteLine("--------------------------------------------------------------------------------");
            for (int i = 0; i < appointments.Count; i++)
            {
                ...
                Console.WriteLine($"{i + 1,-4}| {appt.AppointmentID,-32} | {appt.CustomerID,-30} | {appt.BarberID,-30} | {appt.ServiceID,-30} | {appt.AppointmentDateTime,-19} | {appt.Status}");
            }
            Console.WriteLine("--------------------------------------------------------------------------------");
            ...
            Console.WriteLine("3. CANCELLED_BY_STAFF");
            Console.WriteLine("4. CANCELLED_BY_CUSTOMER");
            Console.Write("Nhap lua chon: ");
            string statusChoice = Console.ReadLine();
            ...
            {
                case "1":
                    newStatus = Entities.AppointmentStatus.SCHEDULED;
                    break;
                case "2":
                ...
                case "3":
                    ...
                    break;
                case "4":
                    newStatus = Entities.AppointmentStatus.CANCELLED_BY_CUSTOMER;
                    break;
                default:
                    Console.WriteLine("Lua chon khong hop le.");
                    return;
        ...
        static async Task AdminLoggedInMenu(string adminName, string adminUserId)
        {
            ...
            while (!exit)
            {
                ...
                {
                    case "1":
                        await ViewAllAppointmentsAdmin();
                        break;
                    case "2":
                        await UpdateAppointmentStatusAdmin();
                        break;
                    case "3":
